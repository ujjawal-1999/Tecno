const glitcher = function(sketch, customId,imgSrcArr, height, width, top, left){
...
// let x= 350;
// console.log(x*100/document.documentElement.clientWidth)
// let y=330
// console.log(y*100/document.documentElement.clientHeight)
  console.log('x,y', x,y)
  ...
  sketch.setup = function(){
    // sketch.background(0);
    const cnv=sketch.createCanvas(x, y);
    cnv.id(customId);
    sketch.clear();
    ...
    // cnv.style('left', left);
    function loadImg(){
        sketch.loadImage(imgSrcArr[item], function(img) {
  ...
  class Glitch {
    ...
    shiftRGB(srcImg) {
        ...
        range = 16;
        destPixels = new Uint8ClampedArray(srcImg.pixels);
        randR = (sketch.floor(sketch.random(-range, range)) * srcImg.width + sketch.floor(sketch.random(-range, range))) * this.channelLen;
        randG = (sketch.floor(sketch.random(-range, range)) * srcImg.width + sketch.floor(sketch.random(-range, range))) * this.channelLen;
        randB = (sketch.floor(sketch.random(-range, range)) * srcImg.width + sketch.floor(sketch.random(-range, range))) * this.channelLen;
    ...
    show() {
        ...
        if (!this.throughFlag) {
            sketch.push();
            sketch.translate((sketch.width - this.imgOrigin.width) / 2, (sketch.height - this.imgOrigin.height) / 2);
        ...
        this.scatImgs.forEach((obj) => {
            ...
            if (sketch.floor(sketch.random(100)) > 80) {
                obj.x = sketch.floor(sketch.random(-this.imgOrigin.width * 0.3, this.imgOrigin.width * 0.7));
                obj.y = sketch.floor(sketch.random(-this.imgOrigin.height * 0.1, this.imgOrigin.height));
